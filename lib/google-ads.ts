...
export async function getCTRWeek() {
  ...
  if (!client || !CUSTOMER_ID || !process.env.GOOGLE_ADS_REFRESH_TOKEN) {
    ...
    return {
      ...
      trend: trends[Math.floor(Math.random() * trends.length)] as 'up' | 'down' | 'neutral'
    };
  }
...
export async function getROASWeek() {
  ...
  try {
    ...
    const previousROAS = previousCost > 0 ? previousConversionsValue / previousCost : 0;
...
export async function getCTRDaily() {
  // Si no tenemos client configurado, usar datos mock directamente
  if (!client || !CUSTOMER_ID || !process.env.GOOGLE_ADS_REFRESH_TOKEN) {
    console.log('Usando datos mock para CTR diario - Google Ads no configurado');
    const mockData = [];
    for (let i = 6; i >= 0; i--) {
      const date = new Date();
  ...
  try {
    ...
    const dailyData = [];
    const dataByDate: { [key: string]: { clicks: number; impressions: number } } = {};
...
export async function getDeviceData() {
  // Si no tenemos client configurado, usar datos mock directamente
  if (!client || !CUSTOMER_ID || !process.env.GOOGLE_ADS_REFRESH_TOKEN) {
    console.log('Usando datos mock para dispositivos - Google Ads no configurado');
    return [
  ...
  try {
    ...
    const deviceData: { [key: string]: { clicks: number; impressions: number } } = {};
    let totalClicks = 0;
    ...
    const result = Object.entries(deviceData).map(([device, data]) => ({
      ...
      percentage: totalClicks > 0 ? Number(((data.clicks / totalClicks) * 100).toFixed(1)) : 0
    }));

    return result.sort((a, b) => b.clicks - a.clicks);
  } catch (error) {
    console.error('Error al obtener datos de dispositivos de Google Ads:', error);
    // Fallback a datos mock
    return [
      { device: 'Mobile', clicks: 1250, impressions: 8500, percentage: 62.5 },
      { device: 'Desktop', clicks: 680, impressions: 4200, percentage: 34.0 },
...
export async function getGeographicData() {
  // Si no tenemos client configurado, usar datos mock directamente
  if (!client || !CUSTOMER_ID || !process.env.GOOGLE_ADS_REFRESH_TOKEN) {
    console.log('Usando datos mock para ubicaciones - Google Ads no configurado');
    return [
      { location: 'Buenos Aires, Argentina', clicks: 847, impressions: 5420, ctr: 15.6 },
      { location: 'São Paulo, Brazil', clicks: 623, impressions: 4100, ctr: 15.2 },
      { location: 'Mexico City, Mexico', clicks: 445, impressions: 3200, ctr: 13.9 },
      { location: 'Madrid, Spain', clicks: 312, impressions: 2800, ctr: 11.1 },
      { location: 'Miami, United States', clicks: 289, impressions: 2150, ctr: 13.4 },
      { location: 'Lima, Peru', clicks: 203, impressions: 1620, ctr: 12.5 },
      { location: 'Bogotá, Colombia', clicks: 178, impressions: 1480, ctr: 12.0 }
    ];
  }
  ...
  try {
    ...
    const locationData: { [key: string]: { clicks: number; impressions: number } } = {};
    ...
    const result = Object.entries(locationData)
      .map(([location, data]) => ({
        ...
        clicks: data.clicks,
        impressions: data.impressions,
        ctr: data.impressions > 0 ? Number(((data.clicks / data.impressions) * 100).toFixed(1)) : 0
      }))
